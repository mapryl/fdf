...
/*   Updated: 2019/08/20 14:43:19 by mflannel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_fdf *fdf_image)
{
	...
	while (curr.x <= p2->x - 1)
	{
		draw_pixel(steep, curr.x, (int)y, fdf_image, get_color(curr, *p1,
					*p2, 1 - (y - (int)y)));
		draw_pixel(steep, curr.x, (int)y + 1, fdf_image, get_color(curr, *p1,
					*p2, y - (int)y));
		y += gradient;
		curr.x++;
	}
...
void	draw_wu(t_point dot1, t_point dot2, t_fdf *fdf_image)
{
	...
	draw_pixel(steep, dot1.x, dot1.y, fdf_image,
			get_color(dot1, dot1, dot2, 1));
	draw_pixel(steep, dot2.x, dot2.y, fdf_image,
